...
import { api, type AgentRunTraceResponse, type AdminStatsResponse, type AdminUserResponse } from "@/lib/api";
...
function BestTeamCard({ name, score }: { name: string; score: number }) {
  return (
    <div className="glass-card rounded-none p-5 flex flex-col gap-2">
      ...
      <p className="text-xs text-gray-500 uppercase tracking-wider font-mono truncate">Best Team · {score.toFixed(1)}/36</p>
    </div>
  );
}
...
function TraceBadge({ status }: { status: string }) {
  const cls = status === "completed"
    ? "bg-accent-neon/15 text-accent-neon border-accent-neon/20"
    : status === "error"
      ? "bg-red-500/15 text-red-400 border-red-500/20"
      : "bg-amber-400/15 text-amber-300 border-amber-400/20";
  return <span className={`text-[10px] px-2 py-0.5 rounded font-mono border ${cls}`}>{status}</span>;
}
...
export function AdminClient() {
  ...
  const [agentRuns, setAgentRuns] = useState<AgentRunTraceResponse[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const runs = await api.adminAgentRuns(session.token);
        setStats(s);
        setUsers(u);
        setAgentRuns(runs);
      } catch (e) {
