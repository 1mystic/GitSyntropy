...
% ============================================================
\title{Predicting Software Team Compatibility via Behavioral
Telemetry and Adaptive Psychometric Profiling}
...
% ============================================================
\begin{abstract}
Predicting software team friction before it manifests in missed
...
Carlo simulation to predict the hire-impact of candidate profiles on
team compatibility.  Evaluated on 46 real GitHub developer profiles
(10,886 commits), the CAT algorithm achieves 100\,\% early-stop rate
at a mean of 5.0 questions (37.5\,\% reduction), and Monte Carlo
variance drops 3.6$\times$ from 100 to 1,000 iterations.
...
alone~\cite{tamburri2019}.  Teams that fail to deliver on schedule
or produce defect-heavy software frequently exhibit patterns of
chronotype mismatch, conflicting communication densities, and
unresolved leadership ambiguity~\cite{catolino2019}.  The cost of
misalignment grows as organizations scale distributed and hybrid
teams, where asynchronous handoffs amplify even modest schedule
incompatibility into multi-day integration delays.
...
orchestrates the full workflow, and Claude~claude-sonnet-4-6 synthesizes
...
The differential weighting scheme used in GitSyntropy's compatibility
engine---integer weights $\{1,2,3,4,5,6,7,8\}$ summing to 36---is
adapted from the \textit{Ashtakoot} (Sanskrit: ``eight attributes'')
system, documented in the classical Indian astronomical treatise
\textit{Brihat Parashara Hora Shastra} (attributed to Parashara Muni,
c.\ 600--800~CE).  In its original context, Ashtakoot is a
structured compatibility scoring system used in Vedic astrology to
assess interpersonal harmony across eight weighted dimensions called
\textit{kootas}.  The eight kootas and their maximum point values
are: Varna (1), Vashya (2), Tara (3), Yoni (4), Graha Maitri (5),
Gana (6), Bhakoot (7), and Nadi (8), totalling 36 \textit{gunas}
(points).  A score of 18 or above is traditionally considered
acceptable, 28 or above excellent.
...
search for hire-impact optimization in the behavioral compatibility
...
adaptive psychometric assessment, and hire-impact simulation in a
unified pipeline.
...
\end{equation}
\begin{equation}
  C(\mathbf{a},\mathbf{b}) = \sum_{d=1}^{8} \text{sim}_d \times w_d
  \;\in [0, 36]
  \label{eq:compat}
\end{equation}
...
\begin{equation}
  \text{conf} = \frac{\text{observed signal count}}{2 \times |\text{dimensions}|}
  \label{eq:conf}
\end{equation}
where the numerator counts non-null dimension scores across both
team members (maximum $2 \times 8 = 16$).
...
Anthropic Claude API (\texttt{claude-sonnet-4-6}) in streaming mode,
...
lookback window using the GitHub REST API.  Starting from 130 curated
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{fig2_crossval_and_distribution}
  \caption{(a)~Cross-modal agreement matrix: Stack Overflow activity
    time vs.\ GitHub commit time (synthetic fallback $n{=}20$; see
    Section~\ref{sec:eval}-B). (b)~Normalized commit-hour histogram
    aggregated over $n{=}46$ developers (10,886 commits); shaded
    bands indicate night hours (navy) and core hours (gold).
    (c)~Chronotype distribution by class ($n{=}46$ real profiles).}
  \label{fig:chrono_dist}
\end{figure*}

To assess cross-modal consistency, we attempted to match 25 GitHub
developer handles to their Stack Overflow accounts and compare
chronotype labels derived from each platform independently.  Most
sampled developers had sparse recent SO activity---senior open-source
maintainers rarely answer new questions---yielding an insufficient
real-data sample ($< 5$ SO posts per user after a 365-day
lookback).  We retain the analysis framework and report synthetic
validation (70\,\% exact match by construction, $n{=}20$) as a
methodological illustration; empirical cross-modal validation
is deferred to future work when a larger set of active SO/GitHub
overlap users can be identified.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig4_cat_early_stop}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig5_monte_carlo_convergence}
  \caption{Monte Carlo convergence. Left: variance of improvement
    estimate across 10 seeds drops $3.6\times$ from $N{=}100$ to
    $N{=}1000$. Right: Euclidean distance of optimal profile from
    the $N{=}5000$ reference stabilises below 1.25 at $N \geq 1000$.}
  \label{fig:mc}
\end{figure}
...
This result reflects the weight structure: Chronotype Sync carries
...
This sample ($n{=}7$ matched) is substantially below the $n \geq 30$
threshold required for reliable confusion matrix estimation.  We
report it as a qualitative proof-of-concept rather than a formal
accuracy study.  A controlled validation study with $n \geq 50$
matched participants is the primary planned next step.
...
We presented GitSyntropy, a multi-agent system that predicts software
...
separation in compatibility scores ($p < 0.001$), validating the
primacy of schedule alignment in the weight model.
